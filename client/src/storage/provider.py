...
class SQLiteStorageProvider:
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('CREATE TABLE IF NOT EXISTS identity (username TEXT PRIMARY KEY, private_key_b64 TEXT)')
            # 2. 密钥管理 (R7)
            cursor.execute('CREATE TABLE IF NOT EXISTS contact_keys (contact_id TEXT PRIMARY KEY, pub_key_b64 TEXT)')
            cursor.execute('CREATE TABLE IF NOT EXISTS session_keys (contact_id TEXT PRIMARY KEY, session_key_b64 TEXT)')
            
            # --- 接口 2：防重放攻击存储表 (核心补充) ---
            # 记录已收到的消息 Nonce，防止黑客截获旧数据包后重复发送
