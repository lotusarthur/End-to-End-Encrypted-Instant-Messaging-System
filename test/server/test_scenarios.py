...
class TestScenarioRunner:
    ...
    def _clean_test_database(self):
        ...
        try:
            # 删除整个数据库文件
            if os.path.exists("test_messaging.db"):
                os.remove("test_messaging.db")
                print("✓ 测试数据库文件已删除")
            else:
                print("✓ 测试数据库文件不存在，无需删除")
        ...
        if success:
            ...
            if not bob_user.is_online:
                ...
                if message_found:
                    ...
                    if offline_message and offline_message.ciphertext_b64 == encrypted_package.ciphertext_b64:
                        print("✓ 离线消息内容正确")
                    else:
                        print("✗ 离线消息内容不正确")
                        return False
